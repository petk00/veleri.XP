            ...
            <transition name="step-fade" mode="out-in">
              ...
              <div v-else-if="step === 2" key="2" class="step">
                <div class="step__head">
                  ...
                  <h2 class="step__title">Svrha nabave</h2>
                  <p class="step__desc">Opišite zašto se ova nabava vrši i čemu služi.</p>
                </div>
                ...
                <div class="step__body">
                  <div class="field">
                    ...
                    <q-input
                      ...
                      type="textarea"
                      outlined autogrow rows="5"
                      counter maxlength="1000"
                      placeholder="npr. Nabava prijenosnog računala za novog zaposlenika u odjelu razvoja..."
              ...
              <!-- ───── STEP 4a: Detalji (s ponudom) ───── -->
              <div v-else-if="step === 4 && form.hasOffer === true" key="4a" class="step">
                <div class="step__head">
                  <div class="step__caption">Korak 4 od {{ currentSteps.length }}</div>
                  <h2 class="step__title">Detalji nabave</h2>
                  <p class="step__desc">Priložite ponudu, unesite iznos i odaberite kategoriju.</p>
                </div>
              ...
              <!-- ───── STEP 4b: Stavke (bez ponude) ───── -->
              <div v-else-if="step === 4 && form.hasOffer === false" key="4b" class="step">
                <div class="step__head">
                  <div class="step__caption">Korak 4 od {{ currentSteps.length }}</div>
                  <h2 class="step__title">Stavke zahtjeva</h2>
                  <p class="step__desc">Navedite što trebate nabaviti.</p>
                </div>
                ...
                <div class="step__body">
                  <div class="review">
                    ...
                    <div class="review__field">
                      ...
                      <button class="review__edit" type="button" @click="step = 2">
                        <q-icon name="edit" size="12px" /> Uredi
                      </button>
                    </div>
                    ...
                    <div v-if="form.hasOffer === true" class="review__field">
                      ...
                      <button class="review__edit" type="button" @click="step = 4">
                        <q-icon name="edit" size="12px" /> Uredi
                      </button>
                    </div>
                  ...
                  <div class="field" style="margin-top: 20px;">
                    <label class="field__label">Komentar / napomena <span class="field__optional">(opcionalno)</span></label>
                    <q-input
                      v-model="form.comment"
                      type="textarea"
                      outlined autogrow rows="3"
                      counter maxlength="500"
                      placeholder="Dodatne napomene za administratora (npr. hitnost, posebni uvjeti isporuke...)"
                      class="field__input field__input--textarea"
                    />
                  </div>
                </div>
...
const currentSteps = computed(() => {
  const base = [
    ...
    { key: 'svrha',  step: 2, title: 'Svrha',  hint: 'Svrha nabave' },
    { key: 'ponuda', step: 3, title: 'Ponuda', hint: 'Imate li ponudu?' },
  ];
  if (form.value.hasOffer === true) {
    return [
      ...base,
      { key: 'detalji', step: 4, title: 'Detalji', hint: 'Iznos i kategorija' },
      { key: 'pregled', step: 5, title: 'Pregled', hint: 'Potvrda i slanje' },
    ];
  }
  if (form.value.hasOffer === false) {
    return [
      ...base,
      { key: 'stavke', step: 4, title: 'Stavke', hint: 'Popis artikala' },
      { key: 'pregled', step: 5, title: 'Pregled', hint: 'Potvrda i slanje' },
    ];
  }
  return [
...
const validateStep = (stepToValidate = step.value) => {
  ...
  if (stepToValidate >= 2 && !form.value.reasonName.trim()) {
    notifyError('Unesite svrhu nabave.');
    return false;
  }
  if (stepToValidate >= 3 && form.value.hasOffer === null) {
    notifyError('Odaberite jednu od opcija.');
    return false;
  }
  if (stepToValidate >= 4 && form.value.hasOffer === true) {
  ...
  if (stepToValidate >= 4 && form.value.hasOffer === false) {
